...
export const getMyAccount = async (query?: { start?: string; end?: string }): Promise<MyAccountResponse> => {
...
export const getAdminNodesUsage = async (
  ...
  return apiFetch(`/admin/${encodeURIComponent(username)}/usage/nodes`, { query });
};
...
export const deleteApiKey = async (id: number, current_password: string): Promise<void> => {
