...
import { EyeIcon, EyeSlashIcon, SparklesIcon, ClipboardIcon } from "@heroicons/react/24/outline";

import { ChartBox } from "components/common/ChartBox";
import { formatBytes } from "utils/formatByte";
import { getMyAccount, getAdminNodesUsage, changeMyAccountPassword } from "service/myaccount";
import useGetUser from "hooks/useGetUser";
import { createApiKey, deleteApiKey, listApiKeys } from "service/myaccount";
import type { AdminApiKey } from "types/ApiKey";
import type { MyAccountResponse, MyAccountUsagePoint, MyAccountNodeUsage } from "types/MyAccount";
...
const StatsCard: React.FC<StatsCardProps> = ({ label, value }) => (
  <Stat p={4} borderWidth="1px" borderRadius="lg" bg="surface.light" _dark={{ bg: "surface.dark" }}>
    ...
    <Modal isOpen={isOpen} onClose={onClose} isCentered>
      ...
      <ModalContent>
          ...
          <VStack spacing={4} align="stretch">
            <Box maxW="420px">
              <InputGroup>
                ...
                <InputRightElement>
                  <IconButton
                    aria-label={showCurrent ? t("admins.hidePassword") : t("admins.showPassword")}
                    size="sm"
                    variant="ghost"
                    icon={showCurrent ? <EyeSlashIcon width={16} /> : <EyeIcon width={16} />}
            ...
            <Box maxW="420px">
              <HStack spacing={2}>
                <InputGroup>
                  ...
                  <InputRightElement>
                    <IconButton
                    aria-label={showNew ? t("admins.hidePassword") : t("admins.showPassword")}
                      size="sm"
                      variant="ghost"
                      icon={showNew ? <EyeSlashIcon width={16} /> : <EyeIcon width={16} />}
        ...
        <ModalFooter>
          ...
          <Button colorScheme="primary" onClick={handleSubmit} isLoading={isSubmitting} isDisabled={!newPassword}>
            {t("save")}
          </Button>
        </ModalFooter>
      </ModalContent>
...
export const MyAccountPage: React.FC = () => {
  ...
  const apiKeysQuery = useQuery<AdminApiKey[]>(["myaccount-api-keys"], listApiKeys);
  ...
  const deleteKeyMutation = useMutation(
    ({ id, current_password }: { id: number; current_password: string }) => deleteApiKey(id, current_password),
    {
      onSuccess: () => {
        queryClient.invalidateQueries("myaccount-api-keys");
        toast({
  ...
  const dailySeries = useMemo(
    () => [{ name: t("myaccount.dailyUsage", "Daily usage"), data: dailyUsagePoints.map((p) => p.used_traffic) }],
    [dailyUsagePoints, t]
  );
  ...
  const perNodeTotal = useMemo(
    () =>
      perNodeUsage.reduce(
        (sum: number, p: MyAccountNodeUsage) => sum + Number(p.used_traffic || 0),
        0
      ),
    [perNodeUsage]
  );
  ...
  return (
    <VStack spacing={4} align="stretch">
      ...
      <Grid templateColumns={{ base: "1fr", md: "repeat(2, 1fr)" }} gap={4}>
        <GridItem>
          ...
          <SimpleGrid columns={{ base: 1, sm: 3 }} spacing={3}>
            <StatsCard label={t("myaccount.usedData")} value={formatBytes(used, 2)} />
            <StatsCard
              label={t("myaccount.remainingData")}
              value={data.data_limit === null ? t("myaccount.unlimited") : formatBytes(remainingData, 2)}
            />
            <StatsCard
              label={t("myaccount.totalData")}
              value={data.data_limit === null ? t("myaccount.unlimited") : formatBytes(totalData, 2)}
            />
          </SimpleGrid>
        </GridItem>
        <GridItem>
          ...
          <SimpleGrid columns={{ base: 1, sm: 3 }} spacing={3}>
            <StatsCard label={t("myaccount.createdUsers")} value={`${data.current_users_count}`} />
            <StatsCard
              label={t("myaccount.remainingUsers")}
              value={data.users_limit === null ? t("myaccount.unlimited") : `${Math.max(remainingUsers, 0)}`}
            />
            <StatsCard
              label={t("myaccount.totalUsers")}
              value={data.users_limit === null ? t("myaccount.unlimited") : `${usersLimit}`}
            />
          </SimpleGrid>
        </GridItem>
      </Grid>
      ...
      <Grid templateColumns={{ base: "1fr", lg: "2fr 1fr" }} gap={4}>
        <GridItem>
          ...
          >
            <Text fontSize="sm" color="gray.500" _dark={{ color: "gray.400" }} mb={3}>
              ...
              <chakra.span fontWeight="semibold">{formatBytes(dailyTotal, 2)}</chakra.span>
            </Text>
        ...
        <GridItem>
          <ChartBox title={t("myaccount.perNodeUsage")} minH="500px">
            <Text fontSize="sm" color="gray.500" _dark={{ color: "gray.400" }} mb={3}>
              {t("nodes.totalUsage", "Total usage")}:{" "}
              <chakra.span fontWeight="semibold">{formatBytes(perNodeTotal, 2)}</chakra.span>
            </Text>
            {perNodeUsage.length > 0 && donutSeries.some((value: number) => value > 0) ? (
              <ReactApexChart
                type="donut"
                height={360}
              ...
              <Tbody>
                {apiKeysQuery.data?.map((key) => (
                  <Tr key={key.id}>
                    ...
                    <Td>{key.created_at ? dayjs(key.created_at).format("YYYY-MM-DD HH:mm") : "-"}</Td>
                    <Td>
                      {key.expires_at ? dayjs(key.expires_at).format("YYYY-MM-DD") : t("myaccount.never")}
                    </Td>
                    <Td>
                      {key.last_used_at
            ...
            <VStack spacing={4} align="stretch">
              {!hasGeneratedKey && (
                <Box>
                  ...
                  <Select value={selectedLifetime} onChange={(e) => setSelectedLifetime(e.target.value)}>
                    ...
                    <option value="forever">{t("myaccount.lifetimeForever")}</option>
                  </Select>
                </Box>
              )}
              {hasGeneratedKey && (
                  ...
                  <HStack>
                    ...
                    <IconButton
                      ...
                      onClick={() => {
                        ...
                        onCopy();
                        toast({ title: t("copied"), status: "success", duration: 1200 });
                      }}
                    />
                  </HStack>
      ...
      <ChangePasswordModal isOpen={modal.isOpen} onClose={modal.onClose} onSubmit={handlePasswordChange} />
        ...
        <ModalContent>
          ...
          <ModalBody>
            <VStack spacing={3} align="stretch">
              ...
              <InputGroup>
                ...
                <InputRightElement>
                  <IconButton
                    aria-label={showDeletePassword ? t("admins.hidePassword") : t("admins.showPassword")}
                    size="sm"
                    variant="ghost"
                    icon={showDeletePassword ? <EyeSlashIcon width={16} /> : <EyeIcon width={16} />}
          ...
          <ModalFooter>
            ...
            <Button
              ...
              onClick={() => {
                if (deleteKeyId !== null) {
                  deleteKeyMutation.mutate({ id: deleteKeyId, current_password: deletePassword } as any);
                }
              }}
            >
              {t("delete")}
